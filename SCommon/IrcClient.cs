...
namespace SCommon
{
    ...
    public class IrcClient : IDisposable
    {
        #region Variables
        // default server
        private string _server = "thinstack.irc.net";

        // default port
        private int _port = 6667;

        // VERSION
        private string clientVersion = "SpawnChat:0.10:C#-.NET_4.5";
        ...
        //With this, will prevent other list commands from being parsed until a different channel is joined.
        public bool alreadyGotList = false;
        ...
        public event EventHandler<KickUserEventArgs> KickUser = delegate { };
        ...
        public event EventHandler<ChannelEmoteEventArgs> ChannelEmote = delegate { };
        ...
        private void Fire_KickUser(KickUserEventArgs o)
        {
            op.Post(x => KickUser(this, (KickUserEventArgs)x), o);
        }
        private void Fire_NoticeMessage(NoticeMessageEventArgs o)
        {
            op.Post(x => NoticeMessage(this, (NoticeMessageEventArgs)x), o);
        }
        private void Fire_ChannelEmote(ChannelEmoteEventArgs o)
        {
            op.Post(x => ChannelEmote(this, (ChannelEmoteEventArgs)x), o);
        }
        ...
        /// </summary>
        /// <param name="channel">Channel to kick user from</param>
        ...
        public void Kick(string channel, string user, string message)
        {
            ...
            {
                try {
                    Send("KICK " + channel + " " + user + " " + message);
                }
        ...
        public void Version(string user)
        {
            if (irc != null && irc.Connected)
            {
                SendNotice(user, " VERSION " + clientVersion);
                //Send("VERSION " + user);
            }
        }
        /// <summary>
        ...
        public void Emote(string channel, string message)
        {
        ...
        private void ParseData(string data)
        {
            ...
            switch (ircCommand)
            {
                ...
                case "NICK": // someone changed their nick
                    ...
                    var newNick = ircData[2].Substring(1);
                ...
                case "PRIVMSG": // message was sent to the channel or as private
                    {
                        ...
                        //else if (message.StartsWith(":ACTION ")) // if it's an emote message
                        //  Fire_ChannelEmote(new ChannelEmoteEventArgs(to, from, message.Substring(9)));
                        else
                        {
                    ...
                    break;
                case "KICK": // someone gettin kicked
                    {
                        var from = ircData[0].Substring(1, ircData[0].IndexOf('!') - 1);
                        var to = ircData[3];
                        var message = "";
                        try
                        {
                            message = JoinArray(ircData, 4);
                        }
                        catch (ArgumentOutOfRangeException e)
                        {
                        }
                        //Console.WriteLine("USER KICK MESSAGE: " + message);
                        Fire_KickUser(new KickUserEventArgs(to, from, message));
                    }
                    break;
                case "PART":
